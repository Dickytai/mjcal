...
function sevenPairsShanten(counts) {
    ...
    // 嚦咕嚦咕變體：
    // 7對 + 1坎 = 17張 = 胡牌 (shanten=-1)
    // 7對 (14張) = 胡牌 (shanten=-1)
    // 6對 + 1坎 + 1孤張 = 16張 = 聽牌 (shanten=0)
    // 5對 + 2坎 = 16張 = 聽牌 (shanten=0)
    // 8對 = 16張 = 聽牌 (shanten=0)
    // 6對 = 12張 = 聽牌 (shanten=0)
    
    // 7對 + 1+坎 = 胡牌
    if (pairs >= 7 && pongs >= 1 && total === 17) return -1;
    
    // 7對 (14張) = 胡牌
    if (pairs === 7 && total === 14) return -1;
    
    // 嚦咕嚦咕變體: 5對+2坎 或 6對+1坎 = 16張 = 聽牌
    // 即 pairs + pongs = 8 既情況
    if (pairs + pongs === 8 && total === 16) return 0;
    
    // 8對 (16張) = 聽牌
    if (pairs === 8 && total === 16) return 0;
    
    // 7對 + 0坎 (但張數唔夠) = 聽牌
    if (pairs === 7 && pongs === 0) return 0;
