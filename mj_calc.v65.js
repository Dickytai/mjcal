...
function sevenPairsShanten(counts) {
    ...
    // ===== 七對子 (7 pairs, must be 7 DIFFERENT pairs) = 14 tiles = 胡牌 =====
    // 4張牌唔可以當2對
    let sevenPairsCount = 0;
    for (let i = 0; i < 34; i++) {
        if (counts[i] === 2 || counts[i] === 3) sevenPairsCount++;  // 2 or 3 = 1 pair
    }
    
    // ===== 嚦咕嚦咕: 7 pairs + 1 pong = 17 tiles = 胡牌 =====
    let pongs = 0;  // 刻子數 (3張)
    for (let i = 0; i < 34; i++) {
        if (counts[i] >= 3) pongs++;
    }
    
    // 總對數 (包括4張=2對既情況 - 用於嚦咕嚦咕)
    let totalPairs = 0;
    for (let i = 0; i < 34; i++) {
    ...
    // ===== 七對子: 7 different pairs (14 tiles) = 胡牌 =====
    // 必須係7個2-3張既牌組合，4張唔可以
    if (sevenPairsCount === 7 && total === 14) return -1;
    
    // ===== 嚦咕嚦咕: 7 pairs + 1 pong = 17 tiles = 胡牌 =====
    // 7對 + 1坎 = 17張
    if (totalPairs >= 7 && pongs >= 1 && total === 17) return -1;
    
    // ===== 聽牌既情況 =====
    
    // 7 different pairs + 0 pong = 14 tiles = 聽牌 (等1張)
    if (sevenPairsCount === 7 && pongs === 0 && total === 14) return 0;
    
    // ===== 嚦咕嚦咕聽牌變體 (16張) =====
    
    // 變體1: 6對 + 1坎 + 1孤張 (等孤張變對)
    if (sevenPairsCount === 6 && pongs === 1 && total === 16) return 0;
    
    // 變體2: 5對 + 2坎 (等7或8變4張=2對)
    // totalPairs = 5 + 2 = 7, pongs = 2, total = 16
    if (sevenPairsCount === 5 && pongs === 2 && total === 16) return 0;
    
    // 8對 = 16張 = 聽牌 (等任何一張變7對)
    if (sevenPairsCount === 0 && totalPairs === 8 && total === 16 && pongs === 0) return 0;
    
    // 6對 = 12張 = 聽牌 (等1張湊成7對)
    if (sevenPairsCount === 6 && total === 12 && pongs === 0) return 0;
    
    // 其他
    const result = 7 - sevenPairsCount;
    return result;
}
...
function quickShanten(counts) {
    ...
    try {
        ...
        // 如果張數少過13張，library可能會返回錯誤既-1
        // 呢種情況下，設為需要既張數 (直到有14張)
        if (total < 13 && shanten <= 0) {
            shanten = 14 - total;
        }
