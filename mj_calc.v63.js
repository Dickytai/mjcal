...
function sevenPairsShanten(counts) {
    ...
    // ===== 嚦咕嚦咕: 7 pairs + 1 pong = 17 tiles = 胡牌 =====
    if (sevenPairsCount + pongs >= 7 && pongs >= 1 && total === 17) return -1;
    ...
    // 8 different pairs (but 4 of a kind can't count as 2 pairs for 七對子)
    // 所以8對 = 16 tiles = 聽牌
    if (totalPairs >= 8 && total === 16 && pongs === 0) return 0;
    
    // 嚦咕嚦咕聽牌: 6 pairs + 1 pong = 15 tiles = 聽牌
    // 或: 5 pairs + 2 pongs = 16 tiles = 聽牌
    if (sevenPairsCount + pongs >= 7 && total >= 15 && total <= 16) return 0;
    
    // 6對 = 聽牌 (等1張湊成7對)
    if (sevenPairsCount === 6 && total === 12) return 0;
