...
function sevenPairsShanten(counts, tileMode = 16) {
    ...
    // 5對 + 3單張 (13張) = 差1張
    if (purePairs === 5 && singles === 3 && total === 13) return 1;
...
function liGuLiGuShanten(counts) {
    // 計算牌型
    // count=2: 1對
    // count=3: 1坎
    // count=4: 可以當2對 或者 1對+1坎(用3張)
    let purePairs = 0;
    let pongs = 0;
    let singles = 0;
    let quadBonus = 0; // 4張牌可以當額外既對
    ...
    // 胡牌：7對 + 1坎 = 17張
    // 需要7對 + 1坎 = 8 (對+坎既總數)
    if (purePairs + pongs === 8 && total === 17) return -1;
    
    // 聽牌變體1: 8對 = 16張 (等8張)
    if (purePairs === 8 && total === 16 && pongs === 0) return 0;
    
    // 聽牌變體2: 6對 + 1坎 + 1孤張 = 16張 (等1張)
    if (purePairs === 6 && pongs === 1 && singles === 1 && total === 16) return 0;
    
    // 聽牌變體3: 5對 + 2坎 = 16張 (等2張)
    if (purePairs === 5 && pongs === 2 && total === 16) return 0;
    
    // 其他情況 - 返回需要既對數
    // 需要7對 + 1坎 = 8
    return 8 - (purePairs + pongs);
}

function liGuLiGuWaiting(counts) {
    // 計算牌型 (包括count=4)
    let purePairs = 0;
    let pongs = 0;
    let singles = 0;
...
function getMinShanten(counts, wildcards, tileMode = 16) {
    ...
    if (tileMode === 13) {
        ...
        const lglgS = liGuLiGuShanten(counts);
...
function calculateWaiting(counts, wildcards, tileMode = 16) {
    ...
    const lglgShanten = liGuLiGuShanten(counts);
    ...
    if (tileMode === 13) {
        ...
        if (lglgShanten <= minShanten && lglgShanten <= spShanten && lglgShanten <= sncShanten && lglgShanten <= tyShanten) {
            if (lglgShanten === 0) return liGuLiGuWaiting(counts);
            return {};
        }
        // 十三幺 before 十六不搭
...
if (command === 'waiting') {
    ...
    for (let i = 0; i < 34; i++) {
        ...
        const newShanten = testWaitingTile(counts, wildcards, i, TILE_MODE);
