...
function getMinShanten(counts, wildcards) {
    ...
    // 因為library可能對特殊牌型判斷唔岩
    if (minS === -1 && (spS === 0 || lglgS === 0)) {
        return Math.min(spS, lglgS);
    }
    
    // 返回較好既結果 (取最小)
    return Math.min(minS, spS, lglgS, sncS, tyS);
}
...
function calculateWaiting(counts, wildcards) {
    ...
    const spShanten = sevenPairsShanten(counts);  // 七對子 (13張)
    const lglgShanten = liGuLiGuShanten(counts);  // 嚦咕嚦咕 (16張)
    const sncShanten = sixteenNoConnectShanten(counts);
    const tyShanten = thirteenYiShanten(counts);
    
    // 如果七對子shanten更好，用七對子既waiting
