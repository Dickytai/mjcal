...
function liGuLiGuShanten(counts) {
    ...
    // 胡牌：7對 + 1坎 = 17張
    if (purePairs === 7 && pongs === 1 && total === 17) return -1;
...
function thirteenYiShanten(counts) {
    ...
    // 計算非幺九牌既數量 (呢啲應該可以組成一組面子)
    let nonYiCount = 0;
    for (let i = 0; i < 34; i++) {
        if (!yiTiles.includes(i)) {
            nonYiCount += counts[i];
        }
    }
    ...
    // 十三幺結構：
    // - 13孤張 (13 tiles)
    // - + 1組面子 (3 tiles) = 16 tiles = 聽牌 (等1張變對)
    // - + 1張湊對 = 17 tiles = 胡牌
...
function getMinShanten(counts, wildcards, tileMode = 16) {
    ...
    if (tileMode === 13) {
        ...
        // 16張模式：七對子、嚦咕嚦咕、十六不搭、十三幺(屋規)
        const spS = sevenPairsShanten(counts);
        const lglgS = liGuLiGuShanten(counts);
        const sncS = sixteenNoConnectShanten(counts);
        const tyS = thirteenYiShanten(counts);
        
        // 如果標準library話-1，但七對子/嚦咕嚦咕話聽牌，用聽牌果個
        if (minS === -1 && (spS === 0 || lglgS === 0)) {
            bestS = Math.min(spS, lglgS);
        } else {
            bestS = Math.min(minS, spS, lglgS, sncS, tyS);
        }
    }
...
function calculateWaiting(counts, wildcards, tileMode = 16) {
    ...
    if (tileMode === 13) {
        ...
        // 16張模式：七對子、嚦咕嚦咕、十三幺、十六不搭
        if (spShanten <= minShanten && spShanten <= lglgShanten && spShanten <= sncShanten && spShanten <= tyShanten) {
            if (spShanten === 0) return sevenPairsWaiting(counts);
            return {};
        }
