...
// 計算七對子shanten
// 嚦咕嚦咕 (Li Gu Li Gu) 判定邏輯
// 台灣麻將：16張手牌 + 1張摸牌 = 17張胡牌
// 牌型：七對子 + 一刻 (7對 + 1坎 = 17張)
...
function sevenPairsShanten(counts) {
    ...
    for (let i = 0; i < 34; i++) {
        if (counts[i] === 2) {
            ...
            differentPairs++;  // 3 counts as a pair
        } else if (counts[i] === 1) {
            singles++;
        }
    }
    
    // 計算刻子數 (3張或以上) - 只計算count >= 3既
    let pongs = 0;
    for (let i = 0; i < 34; i++) {
        if (counts[i] >= 3) pongs++;
    }
    ...
    let totalPairs = 0;
    for (let i = 0; i < 34; i++) {
        totalPairs += Math.floor(counts[i] / 2);
    }
    ...
    // 嚦咕嚦咕: 7對 + 1坎 = 17張 = 胡牌
    // 7 pairs (count=2) + 1 pong (count=3) = 17 tiles
    ...
    // 變體2: 一刻 + 六對 + 單張 = 6對 + 1坎 + 1孤張 = 16張 = 聽1張
    // 6 pairs (count=2) + 1 pong (count=3) + 1 single (count=1)
    if (purePairs === 6 && pongs === 1 && singles === 1 && total === 16) return 0;
    
    // 變體3: 兩刻 + 五對 = 5對 + 2坎 = 16張 = 聽2張
    // 5 pairs (count=2) + 2 pongs (count=3 each)
    if (purePairs === 5 && pongs === 2 && total === 16) return 0;
    
    // 7對 (14張) = 聽牌 (等1張變8對)
    if (purePairs === 7 && total === 14 && pongs === 0) return 0;
    
    // 6對 (12張) = 聽牌 (等1張變7對)
    if (purePairs === 6 && total === 12 && pongs === 0) return 0;
...
function sevenPairsWaiting(counts) {
    ...
    // 七對子聽牌 (7對 = 14張) - 等任何一張變8對
    if (purePairs === 7 && total === 14 && pongs === 0) {
        for (let i = 0; i < 34; i++) {
            if (counts[i] === 2) {
                waitingMap[TILES[i]] = 1;
            }
    ...
    // 6對 (12張) - 等1張變7對
    if (purePairs === 6 && total === 12 && pongs === 0) {
        for (let i = 0; i < 34; i++) {
            if (counts[i] === 2) {
                waitingMap[TILES[i]] = 1;
            }
...
function calculateWaiting(counts, wildcards) {
    ...
    // 如果七對子shanten更好或相同，用七對子既waiting
