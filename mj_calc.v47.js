...
function identifyPatterns(counts) {
    ...
    // 清龍: 同一門123+456+789
    if (seqStarts.includes(0) && seqStarts.includes(3) && seqStarts.includes(6)) {
...
if (command === 'fan') {
    ...
    if (minShanten === 0 || minShanten === -1) {
        ...
        for (let i = 0; i < 34; i++) {
            ...
            // 檢查係咪valid winning hand
            const total = Object.values(testCounts).reduce((a, b) => a + b, 0);
            if (total === 17 && !isValidWinningHand(testCounts)) continue;
            
            // 計算fan
            const patterns = identifyPatterns(testCounts);
            const fanResult = calculateFan(patterns);
            
            results.push({
                tile: TILES[i],
                shanten: fanResult.total >= 0 ? 0 : -1,
                fan: fanResult.total,
